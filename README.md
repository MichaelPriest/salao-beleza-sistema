...
- **Área da empresa cliente**: `/empresa`, `/empresa/unidades`, `/empresa/assinatura`, `/empresa/cobranca` e `/empresa/site`. Essas telas usam somente o `empresaId`/`unidadeId` do contexto do usuário logado. Em `/empresa`, a empresa cadastra razão social, responsável financeiro, email/telefone de cobrança, documento para nota, endereço de cobrança, dia padrão de vencimento e observações da mensalidade. Em `/empresa/cobranca`, a empresa cliente define apenas o método de cobrança da própria mensalidade (cartão, Pix ou boleto), dia de vencimento e dados do responsável financeiro; o gateway e as chaves secret continuam controlados pela plataforma em `/saas-admin/pagamentos`.
