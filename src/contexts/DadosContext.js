...
export function DadosProvider({ children }) {
  ...
  // 🔥 GARANTIR QUE SEMPRE RETORNAMOS ARRAYS
  const dadosSeguros = useMemo(() => ({
    clientes: Array.isArray(clientes.dados) ? clientes.dados : [],
    profissionais: Array.isArray(profissionais.dados) ? profissionais.dados : [],
    servicos: Array.isArray(servicos.dados) ? servicos.dados : [],
    produtos: Array.isArray(produtos.dados) ? produtos.dados : [],
    agendamentos: Array.isArray(agendamentos.dados) ? agendamentos.dados : [],
    atendimentos: Array.isArray(atendimentos.dados) ? atendimentos.dados : [],
    transacoes: Array.isArray(transacoes.dados) ? transacoes.dados : [],
    compras: Array.isArray(compras.dados) ? compras.dados : [],
    caixa: Array.isArray(caixa.dados) ? caixa.dados : [],
    usuarios: Array.isArray(usuarios.dados) ? usuarios.dados : [],
    fornecedores: Array.isArray(fornecedores.dados) ? fornecedores.dados : [],
    notificacoes: Array.isArray(notificacoes.dados) ? notificacoes.dados : [],
  }), [
    clientes.dados,
    profissionais.dados,
    servicos.dados,
  ...
  // 🔥 CAIXA ATUAL COM SEGURANÇA
  const caixaAtual = useCallback(() => {
    const caixaArray = Array.isArray(caixa.dados) ? caixa.dados : [];
    return caixaArray
      .filter(c => c && c.status === 'aberto')
      .sort((a, b) => new Date(b?.dataAbertura || 0) - new Date(a?.dataAbertura || 0))[0] || null;
  }, [caixa.dados]);

  // 🔥 CALCULAR TOTAIS FINANCEIROS COM SEGURANÇA
  const calcularTotaisFinanceiros = useCallback((dataInicio, dataFim) => {
    ...
    const transacoesArray = Array.isArray(transacoes.dados) ? transacoes.dados : [];

    const transacoesFiltradas = transacoesArray.filter(t => {
  ...
  // 🔥 ESTATÍSTICAS DE AGENDAMENTOS COM SEGURANÇA
  const estatisticasAgendamentos = useCallback((data) => {
    const dataStr = data || new Date().toISOString().split('T')[0];

    const agendamentosArray = Array.isArray(agendamentos.dados) ? agendamentos.dados : [];
    const atendimentosArray = Array.isArray(atendimentos.dados) ? atendimentos.dados : [];

    const agendamentosHoje = agendamentosArray.filter(a => a && a.data === dataStr);
    const atendimentosHoje = atendimentosArray.filter(a => a && a.data === dataStr);

    return {
      agendamentos: agendamentosHoje.length,
      atendimentos: atendimentosHoje.length,
      confirmados: agendamentosHoje.filter(a => a && a.status === 'confirmado').length,
      pendentes: agendamentosHoje.filter(a => a && a.status === 'pendente').length,
      emAndamento: atendimentosHoje.filter(a => a && a.status === 'em_andamento').length,
    };
