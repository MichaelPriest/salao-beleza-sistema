...
// VERSÃO COMPLETA CORRIGIDA - ASSINATURA COMO IMAGEM
...
const processarAssinatura = (valor) => {
  ...
  // Se começar com iVBOR (início de PNG base64)
...
const isRespostaAssinatura = (respostaItem) => {
  ...
  // Verificar se o valor parece ser uma imagem base64
  if (typeof valor === 'string') {
    if (valor.startsWith('data:image')) return true;
    if (valor.includes('base64,')) return true;
    if (valor.startsWith('iVBOR')) return true; // PNG base64
    if (valor.startsWith('/9j/')) return true; // JPEG base64
  }
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={ptBR}>
      <Box>
        ...
        {/* Dialog de Detalhes - COM SUPORTE PARA ASSINATURA */}
