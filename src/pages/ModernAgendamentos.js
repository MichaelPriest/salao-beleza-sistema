...
function ModernAgendamentos() {
  const [solicitacoesOnline, setSolicitacoesOnline] = useState([]);
  const [novaSolicitacaoOnline, setNovaSolicitacaoOnline] = useState({ cliente: '', servico: '', dataHora: '' });

  const adicionarSolicitacaoOnline = () => {
    if (!novaSolicitacaoOnline.cliente.trim()) return;
    setSolicitacoesOnline((atuais) => [{ id: Date.now(), ...novaSolicitacaoOnline, status: 'Pendente' }, ...atuais]);
    setNovaSolicitacaoOnline({ cliente: '', servico: '', dataHora: '' });
  };
    ...
    try {
        ...
        <head>
          ...
          <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            body { font-family: 'Segoe UI', Arial, sans-serif; background: #f5f5f5; padding: 30px; font-size: 12px; color: #333; }
            ...
            .header { background: linear-gradient(135deg, #9c27b0 0%, #ff4081 100%); color: white; padding: 30px; text-align: center; position: relative; }
            .header h1 { font-size: 28px; font-weight: 800; margin-bottom: 8px; }
            .header h2 { font-size: 18px; font-weight: 500; margin-bottom: 20px; }
            .header-info { display: flex; justify-content: center; gap: 30px; margin-top: 20px; padding-top: 20px; border-top: 1px solid rgba(255,255,255,0.2); }
            .stats-section { padding: 25px 30px; border-bottom: 1px solid #e0e0e0; }
            .stats-title { font-size: 16px; font-weight: 700; margin-bottom: 15px; padding-left: 12px; border-left: 4px solid #9c27b0; }
            .stats-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 15px; margin-bottom: 20px; }
            .stat-card { padding: 15px; border-radius: 12px; text-align: center; color: white; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); }
            ...
            .financial-summary { background: #f5f5f5; padding: 15px 20px; border-radius: 12px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; }
            .total-value { font-size: 24px; font-weight: 800; color: #9c27b0; }
            .day-card { margin-bottom: 25px; border: 1px solid #e0e0e0; border-radius: 12px; overflow: hidden; }
            .day-header { background: #faf5ff; padding: 15px 20px; display: flex; align-items: center; gap: 15px; border-bottom: 2px solid #9c27b0; }
            ...
            .day-count { background: #9c27b0; color: white; padding: 5px 12px; border-radius: 20px; font-size: 12px; }
            ...
            @media print { body { background: white; padding: 0; } .day-card { break-inside: avoid; } .status-badge, .stat-card { -webkit-print-color-adjust: exact; print-color-adjust: exact; } }
          </style>
        </head>
        <body>
          <div class="report-container">
            <div class="header">
              ...
              <div class="header-info">
                <div><span class="header-info-label">Período</span><br>${dataInicioFormat} - ${dataFimFormat}</div>
                <div><span class="header-info-label">Data de Emissão</span><br>${formatDateTime(new Date())}</div>
              </div>
            </div>
            <div class="stats-section">
              <div class="stats-title">📊 Resumo do Período</div>
              <div class="stats-grid">
                <div class="stat-card"><div class="stat-number">${totalEventos}</div><div class="stat-label">Total</div></div>
                <div class="stat-card agendamentos"><div class="stat-number">${totalAgendamentos}</div><div class="stat-label">Agendamentos</div></div>
                <div class="stat-card atendimentos"><div class="stat-number">${totalAtendimentos}</div><div class="stat-label">Atendimentos</div></div>
                <div class="stat-card confirmados"><div class="stat-number">${totalConfirmados}</div><div class="stat-label">Confirmados</div></div>
                <div class="stat-card pendentes"><div class="stat-number">${totalPendentes}</div><div class="stat-label">Pendentes</div></div>
                <div class="stat-card cancelados"><div class="stat-number">${totalCancelados}</div><div class="stat-label">Cancelados</div></div>
              </div>
              <div class="financial-summary">
                <div><div>Valor Total</div><div class="total-value">R$ ${totalValor.toFixed(2)}</div></div>
                <div>Média Atend.: R$ ${totalAtendimentos > 0 ? (totalValor / totalAtendimentos).toFixed(2) : '0,00'}</div>
                <div>Média Agend.: R$ ${totalAgendamentos > 0 ? (totalValor / totalAgendamentos).toFixed(2) : '0,00'}</div>
              </div>
            </div>
            <div style="padding: 0 30px 30px 30px;">
              <div class="stats-title">📅 Agenda Detalhada</div>
              ${eventosFiltrados.length > 0 ? gerarTabelaEventos() : '<div style="text-align: center; padding: 40px;">Nenhum evento encontrado.</div>'}
            </div>
            <div class="footer">
              <p>Relatório gerado automaticamente pelo Sistema Salão Beleza</p>
              <p>© ${new Date().getFullYear()} Salão Beleza - Todos os direitos reservados</p>
            </div>
          </div>
        </body>
        </html>
      ...
      setTimeout(() => {
        ...
        auditoriaService.registrar('imprimir_relatorio_agenda', {
          ...
          dados: { periodo: periodoRelatorio.tipo, profissional: selectedProfessional, totalEventos: eventosFiltrados.length }
        });
      }, 500);
