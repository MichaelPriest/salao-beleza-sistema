...
const publicos = [
  {
    ...
    text: 'Visão de agenda, vendas, caixa, equipe, estoque, indicadores e crescimento em uma única tela de gestão.',
  },
  {
    title: 'Para a recepção',
    text: 'Agendamentos, confirmações, cadastro de clientes, abertura de atendimento, comanda e comunicação centralizada.',
  },
  {
    title: 'Para profissionais',
    text: 'Agenda individual, histórico de atendimentos, comissões, disponibilidade, anamnese e acompanhamento de performance.',
  },
  {
    title: 'Para clientes',
    text: 'Portal com agendamentos, histórico, pontos, recompensas, notificações, anamnese, perfil e chamados.',
  },
];
...
const melhoriasFuturas = [
  {
    ...
    items: ['Escolha de serviço, profissional e horário pelo cliente', 'Confirmação, reagendamento, cancelamento e lista de espera', 'Pagamento de sinal para reduzir faltas'],
  },
  {
    icon: <WhatsAppIcon />,
    title: 'Automação por WhatsApp',
    items: ['Lembretes automáticos de agendamento', 'Pós-atendimento, aniversário e reativação de clientes', 'Campanhas segmentadas com links de ação'],
  },
  {
    icon: <PointOfSaleIcon />,
    title: 'Comanda digital e caixa',
    items: ['Serviços, produtos, descontos e formas de pagamento na mesma comanda', 'Abertura, sangria, suprimento e fechamento de caixa', 'Baixa de estoque e cálculo de comissão integrados'],
  },
  {
    icon: <LoyaltyIcon />,
    title: 'Pacotes, assinaturas e gift cards',
    items: ['Venda de pacotes de sessões e planos mensais', 'Controle de saldo, validade e consumo', 'Vale-presente com código único e resgate'],
  },
  {
    icon: <PeopleAltIcon />,
    title: 'CRM e campanhas inteligentes',
    items: ['Segmentação de clientes inativos, aniversariantes e VIPs', 'Cupons automáticos por comportamento', 'Ranking de indicações e recompensas automáticas'],
  },
  {
    icon: <MedicalInformationIcon />,
    title: 'Prontuário, fotos e assinatura digital',
    items: ['Linha do tempo do cliente com histórico completo', 'Fotos antes/depois com autorização de uso de imagem', 'Assinatura digital em anamnese e termos'],
  },
  {
    icon: <Inventory2Icon />,
    title: 'Estoque inteligente',
    items: ['Estoque mínimo, alertas e sugestão de compra', 'Consumo automático por serviço/procedimento', 'Curva ABC, giro, validade e custo médio'],
  },
  {
    icon: <BusinessIcon />,
    title: 'SaaS, trial e onboarding',
    items: ['Trial automatizado, checklist de implantação e tour guiado', 'Upgrade/downgrade por plano e recurso', 'Métricas SaaS como MRR, churn, ativação e inadimplência'],
  },
];

const roadmap = [
  { fase: 'Fase 1', title: 'Vender e reter mais', text: 'Agendamento online, WhatsApp, CRM, campanhas automáticas e avaliações.' },
  { fase: 'Fase 2', title: 'Operação profissional', text: 'Comanda digital, fechamento de caixa, pacotes, DRE e estoque automático.' },
  { fase: 'Fase 3', title: 'Experiência premium', text: 'Prontuário, fotos antes/depois, assinatura digital, orçamentos e checkout.' },
  { fase: 'Fase 4', title: 'Escala SaaS', text: 'Trial, onboarding, planos por recurso, billing automático e métricas da plataforma.' },
];
      ...
      <Container maxWidth="lg">
        ...
        >
          ...
          <Box sx={{ p: { xs: 3, md: 6 } }}>
            ...
            <Box sx={{ mt: 5 }}>
              <Typography variant="h4" sx={{ fontWeight: 900, color: '#4a148c', mb: 1, textAlign: 'center' }}>
                Um material para vender para todos os públicos
              </Typography>
              <Typography sx={{ color: 'text.secondary', textAlign: 'center', maxWidth: 850, mx: 'auto', mb: 3 }}>
                O folder também explica o valor da plataforma para dono, recepção, profissionais e clientes finais.
              </Typography>
              <Grid container spacing={2.5}>
                {publicos.map((publico) => (
                  <Grid item xs={12} sm={6} md={3} key={publico.title}>
                    <Paper variant="outlined" sx={{ p: 2.5, height: '100%', borderRadius: 4, borderColor: 'rgba(194, 24, 91, 0.18)' }}>
                      <Typography sx={{ fontWeight: 900, color: '#c2185b', mb: 1 }}>{publico.title}</Typography>
                      <Typography variant="body2" color="text.secondary">{publico.text}</Typography>
                    </Paper>
                  </Grid>
                ))}
              </Grid>
            ...
            <Box sx={{ mt: 5 }}>
              <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} alignItems={{ xs: 'flex-start', md: 'center' }} justifyContent="space-between" sx={{ mb: 3 }}>
                <Box>
                  <Typography variant="h4" sx={{ fontWeight: 900, color: '#4a148c', mb: 1 }}>
                    Próximas funcionalidades para deixar o sistema ainda mais completo
                  </Typography>
                  <Typography sx={{ color: 'text.secondary', maxWidth: 820 }}>
                    Além dos módulos existentes, o folder passa a apresentar uma visão de evolução do produto com recursos de alto impacto comercial e operacional.
                  </Typography>
                </Box>
                <Chip icon={<ChecklistIcon />} label="Roadmap de evolução" color="secondary" sx={{ fontWeight: 900 }} />
              </Stack>
              <Grid container spacing={2.5}>
                {melhoriasFuturas.map((melhoria) => (
                  <Grid item xs={12} md={6} key={melhoria.title}>
                    <Paper sx={{ p: 2.5, height: '100%', borderRadius: 4, bgcolor: '#fff', border: '1px solid rgba(74, 20, 140, 0.12)' }}>
                      <Stack direction="row" spacing={1.5} alignItems="center" sx={{ mb: 1.5 }}>
                        <Box sx={{ color: '#7b1fa2', display: 'flex' }}>{melhoria.icon}</Box>
                        <Typography variant="h6" sx={{ fontWeight: 900 }}>{melhoria.title}</Typography>
                      </Stack>
                      <Stack spacing={0.8}>
                        {melhoria.items.map((item) => (
                          <Typography key={item} variant="body2" color="text.secondary" sx={{ display: 'flex', gap: 1, alignItems: 'flex-start' }}>
            ...
            <Grid container spacing={2.5} sx={{ mt: 5 }}>
              {roadmap.map((item) => (
                <Grid item xs={12} sm={6} md={3} key={item.fase}>
                  <Paper sx={{ p: 2.5, height: '100%', borderRadius: 4, color: '#fff', background: 'linear-gradient(135deg, #6a1b9a 0%, #c2185b 100%)' }}>
                    <Chip label={item.fase} size="small" sx={{ bgcolor: 'rgba(255,255,255,0.18)', color: '#fff', fontWeight: 900, mb: 1.5 }} />
                    <Typography sx={{ fontWeight: 900, mb: 1 }}>{item.title}</Typography>
            ...
            <Grid container spacing={2.5} sx={{ mt: 5 }}>
              <Grid item xs={12} md={8}>
                <Paper sx={{ p: 3, borderRadius: 5, bgcolor: '#f8f2ff' }}>
                  <Stack direction="row" spacing={1.5} alignItems="center" sx={{ mb: 1.5 }}>
                    <PhotoCameraIcon sx={{ color: '#7b1fa2' }} />
                    <Typography variant="h5" sx={{ fontWeight: 900, color: '#4a148c' }}>
                      Ideias para transformar o folder em kit comercial
                    </Typography>
                  </Stack>
                  <Grid container spacing={1.5}>
                    {['Versão curta de 1 página', 'Versão completa imprimível', 'Tabela de planos e módulos', 'Prints/mockups das telas', 'QR Code para demonstração', 'CTA com WhatsApp real', 'PDF para envio ao cliente', 'Benefícios por perfil de usuário'].map((item) => (
              ...
              <Grid item xs={12} md={4}>
                <Paper sx={{ p: 3, height: '100%', borderRadius: 5, textAlign: 'center', border: '2px dashed rgba(156, 39, 176, 0.3)' }}>
                  <QrCode2Icon sx={{ fontSize: 64, color: '#7b1fa2', mb: 1 }} />
                  <Typography sx={{ fontWeight: 900, color: '#4a148c', mb: 1 }}>Espaço para QR Code</Typography>
                  <Typography variant="body2" color="text.secondary">
                    Use para direcionar para demonstração, WhatsApp, página pública do salão ou cadastro de teste.
                  </Typography>
                </Paper>
              </Grid>
            </Grid>

            <Grid container spacing={4} sx={{ mt: 5 }} alignItems="stretch">
              <Grid item xs={12} md={7}>
